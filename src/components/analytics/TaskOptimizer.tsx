...
export const TaskOptimizer: React.FC<TaskOptimizerProps> = ({ tasks, cognitiveLoad, energyScore }) => {
    const optimizedTasks = useMemo(() => {
        return optimizeTaskList(tasks.filter(t => t.status !== 'completed'), cognitiveLoad, energyScore);
    }, [tasks, cognitiveLoad, energyScore]);

    // Check if any changes were made
    const hasChanges = optimizedTasks.length !== tasks.filter(t => t.status !== 'completed').length ||
